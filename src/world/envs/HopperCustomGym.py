...
class HopperCustomEnv(MujocoEnv, utils.EzPickle):
    r"""
    ## Description
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        rotate_reward_weight: float = 0.2,
        ctrl_cost_weight: float = 1e-3,
        healthy_reward: float = 1e-1,
        terminate_when_unhealthy: bool = True,
        healthy_state_range: Tuple[float, float] = (-100.0, 100.0),
        healthy_z_range: Tuple[float, float] = (0.2, float("inf")),
        healthy_angle_range: Tuple[float, float] = (-float("inf"), float("inf")),
        ...
        **kwargs,
        ...
        self._rotate_reward_weight = rotate_reward_weight
        ...
        self.init_qpos = np.array([0.0, 1.0, 0.0, 1.2, -2, 1])
    ...
    def is_healthy(self):
        ...
        #print(f"healthy_state: {healthy_state}, healthy_z: {healthy_z}, healthy_angle: {healthy_angle}")
        
        #print(f"z: {z}, angle: {angle}")
    ...
    def step(self, action):
        ...
        #print(f"Torques: thigh={action[0]:.3f}, leg={action[1]:.3f}, foot={action[2]:.3f}")
    ...
    def _get_rew(self, x_velocity: float, action):
        ...
        if abs(x_velocity) < 0.2:
            forward_reward -= self._forward_reward_weight * (0.2 - abs(x_velocity))  # penalize slow speeds
        #print(f"x_velocity: {x_velocity} / forward_reward: {forward_reward}")
        healthy_reward = self.healthy_reward
        angle_vel = self.data.qvel[2]  # rooty angular velocity
        rotation_penalty = self._rotate_reward_weight * angle_vel**2  # Tune the coefficient

        rewards = forward_reward + healthy_reward - rotation_penalty
        ...
        reward_info = {
            ...
            "reward_rotation_penalty": -rotation_penalty,
        }
        
        #print(f"reward: {reward}, forward_reward: {forward_reward}, healthy_reward: {healthy_reward}, ctrl_cost: {ctrl_cost}")
